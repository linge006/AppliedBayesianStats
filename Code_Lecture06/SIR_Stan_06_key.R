...
SIR_model <- stan_model(model_code=SIR_neg_bin, model_name="SIR_distribution")
...
fit_SIR_distribution <- sampling(SIR_model,
                           ...
                           warmup = 1e3, iter = 2e3,
                           chains = 1)
ODE_out_rs <- summary(fit_SIR_distribution, pars=c("beta","gamma","phi_inv","phi","R0","recovery_time","pred_cases","lp__"))$summary

write.table(summary(fit_SIR_distribution)$summary, "SIR_ODE.csv", sep=",")
pdf("SIR_ODE_chns.pdf", width=11, height=8.5)
traceplot(fit_SIR_distribution, pars=c("beta","gamma","phi_inv","phi","R0","recovery_time","pred_cases","lp__"))
dev.off()

#

pairs(fit_SIR_distribution, pars=c("beta","gamma","phi_inv","phi","R0","recovery_time"))
